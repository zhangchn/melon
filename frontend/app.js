...
class App {
  ...
  _handleKeyboard(e) {
    ...
    if (mod && e.key === 'f') {
      ...
      // Update image viewer if visible and new selection has preview
      this._updateImageViewerForSelection();
  ...
  // Update image viewer if visible and newly selected node has a preview
  _updateImageViewerForSelection() {
    const imageViewer = d3.select('#image-viewer');
    if (imageViewer.classed('hidden')) return;
    
    const selectedNode = this.chart.selectedNode;
    if (!selectedNode || !selectedNode.data.preview_url) {
      // No preview for this node - close the viewer
      imageViewer.classed('hidden', true);
      return;
    }
